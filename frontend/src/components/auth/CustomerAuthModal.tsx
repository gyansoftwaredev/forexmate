...
export default function CustomerAuthModal({ isOpen, onClose, onSuccess }: CustomerAuthModalProps) {
  ...
  const handleRegOtpChange = (index: number, val: string) => {
    ...
    if (val && index < 5) {
      const nextInput = document.getElementById(`modal-reg-otp-${index + 1}`);
      if (nextInput) nextInput.focus();
    }
  };
  ...
  const handleRegOtpKeyDown = (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Backspace' && !regOtpValues[index] && index > 0) {
      const prevInput = document.getElementById(`modal-reg-otp-${index - 1}`);
      if (prevInput) prevInput.focus();
    }
  };
  ...
  // Step 1: Send Mobile OTP for Sign Up
  ...
  const handleRegisterVerifyAndSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (payload?.access_token && payload?.user) {
        login(payload.access_token, payload.user);
        setSuccessMessage('Account verified & created! Proceeding...');
        setIsSuccess(true);
        setTimeout(() => {
          onSuccess(payload.user);
        }, 600);
